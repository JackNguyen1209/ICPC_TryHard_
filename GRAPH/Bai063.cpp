...
public:
	...
	ll query(ll l, ll r, ll ql, ll qr, ll si)
	{
		...
		if (ql <= l && r <= qr)
			return st[si].s;
		ll m = (l + r) >> 1;
		return query(l, m, ql, qr, si * 2) +
			query(m + 1, r, ql, qr, si * 2 + 1);
	}
...
public:
	...
	ll get(ll u, ll v)
	{
		...
		while (true)
		{
			...
			if (!ispar(l, uu) || uu == l)
			{
				ma += st.query(pos[l], pos[u]);
				break;
			}
			else
			{
				ma += st.query(pos[uu], pos[u]);
				u = up[uu][0];
			}
		}
		while (true)
		{
			...
			if (!ispar(l, vv) || vv == l)
			{
				ma += st.query(pos[l], pos[v]);
				break;
			}
			else
			{
				ma += st.query(pos[vv], pos[v]);
				v = up[vv][0];
			}
		}
		return ma - st.query(pos[l], pos[l]);
	}
	graph(ll nn)
	{
		n = nn;
